...
// ✅ Forgot Password (Dev Mode → no email, just console + response)
export const forgotPassword = async (req, res) => {
  try {
    ...
    // ⚠️ Dev mode → log instead of email
    console.log("🔑 Reset URL (send via email in prod):", resetUrl);

    // Also return in response for testing
    res.json({
      message: "Reset link generated (check server logs)",
      resetUrl, // ⚠️ remove this in production
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
